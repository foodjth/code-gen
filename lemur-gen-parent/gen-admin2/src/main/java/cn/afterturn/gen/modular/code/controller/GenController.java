...
import cn.afterturn.gen.core.util.BootUtil;
...
public class GenController {
    ...
    public ResponseModel queryDatabses(DbInfoModel entity, RequestModel form) {
        try {
            ...
            ConnectionUtil.init(entity.getDbDriver(), entity.getDbUrl(), entity.getDbUserName(), entity.getDbPassword());
    ...
    public void genTableCode(Integer tableId, String localPath, String encoded, GenerationEntity ge, HttpServletRequest req,
        ...
        for (int i = 0; i < templateList.size(); i++) {
            final int index = i;
            fileList.addAll(ParseFactory.getParse(templateList.get(i).getTemplateType()).parse(ge, tableEntity, new ArrayList<String>() {
                {
     ...
     * 本地输出代码
     */
    private void writeThisFileList(String localPath, String encoded, List<String> fileList, List<TemplateModel> templateList,
            GenerationEntity ge) {
    ...
    private void downThisFileList(HttpServletResponse res, List<String> fileList, List<TemplateModel> templateList, GenerationEntity ge) {
        ...
        try {
            ...
            for (int i = 0; i < fileList.size(); i++) {
                out.putNextEntry(
                        new ZipEntry(getPackagePath(templateList.get(i), ge) + File.separator + getFileName(templateList.get(i), ge)));
    ...
    private String getFileName(TemplateModel templateModel, GenerationEntity ge) {
        if (templateModel.getFileName().endsWith("js")) {
            return String.format(templateModel.getFileName(), BootUtil.uncap(ge.getEntityName()));
        } else if (templateModel.getFileName().endsWith("html")) {
            return String.format(templateModel.getFileName(), BootUtil.uncap(ge.getEntityName()));
        } else if (templateModel.getFileName().endsWith("jsp")) {
            return String.format(templateModel.getFileName(), BootUtil.uncap(ge.getEntityName()));
    ...
    private String getPackagePath(TemplateModel templateModel, GenerationEntity ge) {
        String uncapName = File.separator + BootUtil.uncap(ge.getEntityName());
        if (templateModel.getFileName().endsWith("js")) {
            return ge.getJsPackage().replaceAll("\\.", "\\/") + (StringUtils.isNotEmpty(templateModel.getTemplatePath())
                    ? "/" + templateModel.getTemplatePath().replaceAll("\\.", "\\/") : "") + uncapName;
        } else if (templateModel.getFileName().endsWith("html")) {
            return ge.getHtmlPackage().replaceAll("\\.", "\\/") + (StringUtils.isNotEmpty(templateModel.getTemplatePath())
                    ? "/" + templateModel.getTemplatePath().replaceAll("\\.", "\\/") : "") + uncapName;
        } else if (templateModel.getFileName().endsWith("xml")) {
            return ge.getXmlPackage().replaceAll("\\.", "\\/") + (StringUtils.isNotEmpty(templateModel.getTemplatePath())
                    ? "/" + templateModel.getTemplatePath().replaceAll("\\.", "\\/") : "") + uncapName;
        } else {
            return ge.getCodePackage().replaceAll("\\.", "\\/") + (StringUtils.isNotEmpty(templateModel.getTemplatePath())
